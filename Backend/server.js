...
app.get('/api/employees', async (req, res) => {
  try {
    ...
    const employees = result.rows.map(employee => ({
      ...
      aadharFile: employee.aadhar_file ? `http://174.129.80.224:3096/${employee.aadhar_file}` : null,
      panNo: employee.pan_no,
      panFile: employee.pan_file ? `http://174.129.80.224:3096/${employee.pan_file}` : null,
      pfNo: employee.pf_no,
      ...
      signature: employee.signature_file ? `http://174.129.80.224:3096/${employee.signature_file}` : null,
    ...
    employees.forEach(employee => {
      employee.educationDetails = educationResult.rows
        ...
        .map(edu => ({
          ...
          year: edu.year,
          score: edu.score,
          doc: edu.doc_path ? `http://174.129.80.224:3096/${edu.doc_path}` : null
        }));
      ...
      employee.employmentDetails = employmentResult.rows
        ...
        .map(emp => ({
          ...
          doc: emp.doc_path ? `http://174.129.80.224:3096/${emp.doc_path}` : null
        }));
    });
